...
:func:`puglRegisterDropType` must be called during setup to register a MIME type.
...
.. code-block:: c
   ...
   puglRegisterDropType(view, "application/zip");
...
.. code-block:: c
   ...
   onDataOffer(PuglView* view, const PuglDataOfferEVent* event)
   {
     ...
     uint32_t      numTypes  = puglGetNumClipboardTypes(view, clipboard);
...
A view region must also be given,
