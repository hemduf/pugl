...
EM_JS(int, puglBrowserInstallInput, (const char* selector, uintptr_t token), {
  ...
  handlers.pointerDown = (event) => {
    ...
    // The hidden textarea owns keyboard focus. Prevent the canvas pointer
    // default action from immediately taking it back and synthesizing a
    // spurious PUGL_FOCUS_OUT/PUGL_FOCUS_IN pair on every click.
    if (event.cancelable) {
      event.preventDefault();
    }
